...
function customEntries(obj) {
  ...
  const FormList = Object.entries(obj);
  return FormList;
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const ObjAss = Object.assign(obj1, obj2);
  Object.freeze(ObjAss);
  if (!Object.isFrozen(ObjAss)) {
    return "Помилка:Обєкт не заморожено";
  }
  return ObjAss;
...
function checkAndPreventExtensions(obj) {
  ...
  const pora = Object.isExtensible(obj);
  const hora = Object.preventExtensions(pora);
  return hora;
...
function sealAndCheck(obj) {
  ...
  const poka = Object.seal(obj);
  const moka = Object.isSealed(poka);
  return moka;
  // Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
...
function convertArrayToObj(arr) {
  ...
  const hasDuplicates = Object.values(obj).some((value, index, self) => {
    return self.indexOf(value) !== index;
  });
  
  if (hasDuplicates) {
    console.log("Масив має дубльовані ключі!");
  }
